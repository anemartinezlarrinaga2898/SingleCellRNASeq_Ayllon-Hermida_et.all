...
pdf(paste(path.guardar,"A0060_Graficos.QualityControl.pdf",sep="/"),width = 15,height = 20)
...
pdf(paste(path.guardar,"A0060_Graficos.After.QualityControl.pdf",sep="/"),width = 15,height = 20)
...
ggsave(Umap.doublet,filename = paste(path.guardar,"A0060_DoubletIdentification.pdf",sep = "/"),width = 10,height = 10)

data.filtered <- data.filtered$data

proportions.doublets <- as.data.frame(table(data.filtered$DoubletClassification))
write_csv(proportions.doublets,paste(path.guardar,"A0060_Proportions.Doublets.csv",sep="/"))
...
ggsave(ElbowPlot,filename = paste(path.guardar,"A0060ElbowPlot.png",sep = "/"),width = 10,height = 10)
...
pdf(paste(path.guardar,"A0060_Umaps.DifferentResolutions.pdf",sep="/"),width = 15,height = 20)
Umaps.totales
dev.off()
...
write_csv(number.cluster.for.each.resolution,paste(path.guardar,"NumberOfClustersInEachResolution.csv",sep="/"))

saveRDS(data,paste(path.guardar,"A0060_SeuratObjectProcessed.rds",sep="/"))
